<div class="bg-gray-200 h-full">
    <?php include("componentes/content_cnh.php") ?>
            ...
            <div
                ...
                <form method="post" id="confirmar">
                    ...
                    <div class="p-6 text-center">
                        <svg class="h-[60px] mx-auto" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                            fill="#000000">
    ...
    <div data-te-modal-init
        class="fixed top-0 left-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none"
        id="negar_cnh" tabindex="-1" aria-labelledby="negar_cnhTitle" aria-modal="true" role="dialog">
        <div data-te-modal-dialog-ref
            class="pointer-events-none relative flex min-h-[calc(100%-1rem)] w-auto translate-y-[-50px] items-center opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:min-h-[calc(100%-3.5rem)] min-[576px]:max-w-[500px]">
            <div
                class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-none dark:bg-neutral-600">
                <div
                    class="flex flex-shrink-0 items-center justify-between rounded-t-md border-b-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
                    <h5 class="text-xl hi font-medium leading-normal text-neutral-800 dark:text-neutral-200">
                        Confirmar CNH Do Motoboy Nº <span id="nmr_motoboy_negar">
                    </h5>
                    <button type="button"
                        class="box-content rounded-none border-none hover:no-underline hover:opacity-75 focus:opacity-100 focus:shadow-none focus:outline-none"
                        data-te-modal-dismiss aria-label="Close">
                ...
                <form method="post" id="negar">
                    <input type="text" value="" id="id_motoboy_negar" class="hidden">
                    <div class="p-6 text-center">
                        <svg class="h-[60px] mx-auto" cfill="#000000" viewBox="0 0 24 24"
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400" id="desc_cnh_negar"></h3>
                        <button data-te-modal-dismiss type="button"
                            class="text-white bg-red-600 hover:bg-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">Não</button>
                        <button data-te-modal-dismiss type="submit"
                            class="text-white bg-[#002d2b] hover:text-[#00fff4] rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Sim</button>
                    </div>
                </form>
            </div>
    ...
    $(document).ready(function() {
        ...
        $('#table').on('click', '.cancelar_cnh', function(e) {
            e.preventDefault();
            var id_motoboy = this.id;
            var new_id = id_motoboy.replace("cancelar-", "");
            ...
            $.post("<?php echo site_url("grente/pegar_info_motoboy") ?>", {
                ...
                function(data) {
                    ...
                    $("#nmr_motoboy_negar").html(dados[0]["idmotoboy"]);
                    $("#desc_cnh_negar").html("Realmente Deseja Negar Este CNH?");
                    $("#id_motoboy_negar").val(dados[0]["idmotoboy"]);
                    const myModalEl = document.getElementById("negar_cnh");
            ...
            $("#confirmar").submit(function(e) {
                ...
                $.post("<?php echo site_url("grente/confirmar_cnh") ?>", {
                    ...
                    function(data) {
                        ...
                        $.post("<?php echo site_url("grente/sendEmailCnh_Aceito") ?>", {
                            email: $email
                        },function(data){})
        ...
        $("#negar").submit(function(e) {
            e.preventDefault();
            $.post("<?php echo site_url("grente/negar_cnh") ?>", {
                    id: $("#id_motoboy_negar").val()
                },
                function(data) {
                    $("#loader_gr").show();
                    $("html").addClass("overflow-hidden");
                    $.post("<?php echo site_url("grente/pegar_info_motoboy") ?>", {
                        ...
                        dados = JSON.parse(data);
                        $email = dados[0]["email_motoboy"];
                        $.post("<?php echo site_url("grente/sendEmailCnh_Negado") ?>", {
                            email: $email
                        },function(data){})
